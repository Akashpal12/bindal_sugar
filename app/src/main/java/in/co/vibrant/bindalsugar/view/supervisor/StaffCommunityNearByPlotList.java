...
public class StaffCommunityNearByPlotList extends AppCompatActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try
        {
            setContentView(R.layout.near_by_plot_map_view_community);
            Toolbar toolbar = findViewById(R.id.toolbar);
            setSupportActionBar(toolbar);
            setTitle(getString(R.string.MENU_NEARBY_PLOT));
            context= StaffCommunityNearByPlotList.this;
            ...
            mapFragment.getMapAsync(StaffCommunityNearByPlotList.this);
    ...
    {
        Intent intent=new Intent(context, StaffCommunityList.class);
    ...
    private class GetMapData extends AsyncTask<String, Void, Void> {
        ...
        protected Void doInBackground(String... params) {
            try {
                ...
                //request1.addProperty("lat", "27.8788331");
                //request1.addProperty("lng", "81.137597");
    ...
    public void drawPlot(JSONArray plotFarmerShareArray) throws JSONException {
        if(plotFarmerShareArray.length()>0)
        {
            for (int i = 0; i < plotFarmerShareArray.length(); i++) {
                ...
                CustomInfoWindowAdapter adapter = new CustomInfoWindowAdapter(StaffCommunityNearByPlotList.this);
    ...
    private class GetGrowerDataList extends AsyncTask<String, Void, Void> {
        String message;
        private ProgressDialog dialog = new ProgressDialog(context);
        ...
        protected Void doInBackground(String... params) {
            try {
                TelephonyManager telephonyManager = (TelephonyManager) getSystemService(Context.TELEPHONY_SERVICE);
                SoapObject request1 = new SoapObject(APIUrl.NAMESPACE, APIUrl.method_checkPolygonGrower_New);
                request1.addProperty("imeino",new GetDeviceImei(context).GetDeviceImeiNumber());
                request1.addProperty("lat", ""+lat);
                request1.addProperty("lng", ""+lng);
                /*request1.addProperty("lat", "27.81116");
                request1.addProperty("lng", "81.03607");*/
                request1.addProperty("Divn", userDetailsModels.get(0).getDivision());
                Log.d("", "doInBackground: " + request1);
                SoapSerializationEnvelope envelope = new SoapSerializationEnvelope(SoapEnvelope.VER11);
                envelope.dotNet = true;
                ...
                HttpTransportSE androidHttpTransport = new HttpTransportSE(APIUrl.BASE_URL, 200000);
                androidHttpTransport.debug = true;
                androidHttpTransport.call(APIUrl.SOAP_ACTION_checkPolygonGrower_New, envelope);
                if (envelope.bodyIn instanceof SoapFault)
                {
                    SoapFault sf = (SoapFault) envelope.bodyIn;
                    message = sf.getMessage();
                    ...
                    SoapObject result = (SoapObject) envelope.bodyIn;
                    message = result.getPropertyAsString("checkPolygonGrower_NewResult").toString();
                }
            } catch (SecurityException e) {
                Log.e("Exception", e.getMessage());
                message = e.getMessage();
                ...
                if(obj.getString("API_STATUS").equalsIgnoreCase("OK"))
                {
                    ...
                    if(jsonArray.length()==0)
                    {
                        ...
                        for(int i=0;i<jsonArray.length();i++)
                        {
                            ...
                            intent.putExtra("villCode",jsonObject.getString("VILLAGE_ID"));
                            intent.putExtra("villName",jsonObject.getString("G_VILLAGE"));
                            intent.putExtra("growCode",jsonObject.getString("G_CODE"));
                            intent.putExtra("growerName",jsonObject.getString("G_NAME"));
                            intent.putExtra("growerFatherName",jsonObject.getString("GF_NAME"));
