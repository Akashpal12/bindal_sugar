...
import in.co.vibrant.bindalsugar.adapter.ListGetVillageWiseVarietySummeryAdapter;
...
public class VillageWiseVarietySummery extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle("Village Wise Variety Summary");
        toolbar.setTitle("Village Wise Variety Summary");
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setDisplayShowHomeEnabled(true);
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent=new Intent(VillageWiseVarietySummery.this, RaSurveyDashboard.class);
                startActivity(intent);
                finish();
            }
        });
        if(new InternetCheck(VillageWiseVarietySummery.this).isOnline())
        {
            new GetDataList().execute(factory,SupvcodeFrom,SupvcodeTo,VillageFrom,VillageTo,vrtfrom,vrtto,FromDate,ToDate);
        }
        else
        {
            Toast.makeText(VillageWiseVarietySummery.this,getString(R.string.oops_connect_your_internet),Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void AlertPopUp(String msg) {
        AlertDialog.Builder alertDialog = new AlertDialog.Builder(
                VillageWiseVarietySummery.this);
    ...
    private class GetDataList extends AsyncTask<String, Void, Void> {
        String message;
        private ProgressDialog dialog = new ProgressDialog(VillageWiseVarietySummery.this);

        @Override
        protected void onPreExecute() {
            super.onPreExecute();
            dialog = ProgressDialog.show(VillageWiseVarietySummery.this, getString(R.string.app_name),
                    "Please Wait ", true);
            dialog.show();
        }
        ...
        protected Void doInBackground(String... params) {
            try {
                ...
                SoapObject request1 = new SoapObject(APIUrl.NAMESPACE, APIUrl.method_GetSupervisorVarietyAreaNew);
                ...
                androidHttpTransport.call(APIUrl.SOAP_ACTION_GetSupervisorVarietyAreaNew, envelope);
                if (envelope.bodyIn instanceof SoapFault) {
                    SoapFault sf = (SoapFault) envelope.bodyIn;
                    message = sf.getMessage();
                } else {
                    SoapObject result = (SoapObject) envelope.bodyIn;
                    message = result.getPropertyAsString("GetSupervisorVarietyAreaNewResult").toString();
                }
        ...
        protected void onPostExecute(Void result) {
            ...
            try{
                ...
                if(jsonArray.length()==0)
                {
                        ...
                        for(int i=0;i<villArray.length();i++) {
                            ...
                            if (i > 0) {
                                ...
                                if (jsonObject1.getInt("SUPVCODE") == jsonObject.getInt("SUPVCODE")) {
                                    ...
                                    supData.put("VCODE",jsonObject.getString("VRCODE"));
                                    supData.put("VNAME",jsonObject.getString("VRNAME"));
                    ...
                    GridLayoutManager manager = new GridLayoutManager(VillageWiseVarietySummery.this, 1, RecyclerView.VERTICAL, false);
                    //recyclerView.setLayoutManager(manager);
                    //gridLayoutManager=new GridLayoutManager(MainActivity.this,8,RecyclerView.HORIZONTAL,true);
                    recyclerView.setHasFixedSize(false);
                    recyclerView.setLayoutManager(manager);
                    ListGetVillageWiseVarietySummeryAdapter listGetCropTypeSummeryAdapter =new ListGetVillageWiseVarietySummeryAdapter(VillageWiseVarietySummery.this,jsonArray1);
            ...
            catch (JSONException e)
            {
                if(dialog.isShowing())
                    dialog.dismiss();
                new AlertDialogManager().AlertPopUpFinish(VillageWiseVarietySummery.this,message);
                //AlertPopUpFinish(message);
            }catch (Exception e)
            {
                new AlertDialogManager().AlertPopUpFinish(VillageWiseVarietySummery.this,"Error:"+e.toString());
            }
        }
    }
