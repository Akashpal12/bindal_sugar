...
public class RaSupervisorWithVarietyReport extends AppCompatActivity  {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context= RaSupervisorWithVarietyReport.this;
        setTitle("Supervisor  Varity Wise Planting");
        toolbar.setTitle("Supervisor Varity Wise Planting");
        ...
        fromdate_select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                DatePickerDialog dpd = new DatePickerDialog(RaSupervisorWithVarietyReport.this, new DatePickerDialog.OnDateSetListener() {
                    @Override
    ...
    class TargetList extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            try {
                String url= APIUrl.BASE_URL_CANE_DEVELOPMENT +"/PLANTCDOVILLAGEVARIETYWISESEEDQTYACHIEVED";
        ...
        protected void onPostExecute(String sResponse) {
            ...
            try {
                ...
                if(jsonArray.length()>0)
                {
                    ...
                    currentVillageCode = jsonObject1.getString("UCODE");
                    currentVillageName = jsonObject1.getString("UNAME");
                    List<Map<String,String>> target=new ArrayList<>();
                    for (int i = 0; i < jsonArray.length(); i++)
                    {
                        JSONObject jsonObject=jsonArray.getJSONObject(i);
                        if(currentVillageCode.equalsIgnoreCase(jsonObject.getString("UCODE")))
                        {
                            ...
                            map.put("TRGSEEDQTY",jsonObject.getString("TRGSEEDQTY"));
                            map.put("TRGGROW",jsonObject.getString("TRGGROW"));
                            map.put("ACHIVSEEDQTY",jsonObject.getString("ACHIVSEEDQTY"));
                            map.put("ACHIVGROW",jsonObject.getString("ACHIVGROW"));
                            target.add(map);
                        }
                        else
                        {
                            if(target.size()>0)
                            {
                                ...
                                for(int j=0;j<target.size();j++)
                                {
                                    ...
                                    map1.put("UCODE",target.get(j).get("UCODE"));
                                    map1.put("UNAME",target.get(j).get("UNAME"));
                                    map1.put("VRCODE",target.get(j).get("VRCODE"));
                                    map1.put("VRNAME",target.get(j).get("VRNAME"));
                                    map1.put("TRGSEEDQTY",target.get(j).get("TRGSEEDQTY"));
                                    map1.put("TRGGROW",target.get(j).get("TRGGROW"));
                                    map1.put("ACHIVSEEDQTY",target.get(j).get("ACHIVSEEDQTY"));
                                    map1.put("ACHIVGROW",target.get(j).get("ACHIVGROW"));
                                    target1.add(map1);
                                }
                                ...
                                currentVillageCode = jsonObject.getString("UCODE");
                                currentVillageName = jsonObject.getString("UNAME");
                                target.clear();
                            }
                            Map<String,String> map =new HashMap<String, String>();
                            map.put("UCODE",jsonObject.getString("UCODE"));
