...
import in.co.vibrant.bindalsugar.adapter.ListGetVarietyGroupSummeryAdapter;
...
public class VarietyGroupSummery extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle(getString(R.string.MENU_VARIETY_GROUP_SUMMARY));
        toolbar.setTitle(getString(R.string.MENU_VARIETY_GROUP_SUMMARY));
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setDisplayShowHomeEnabled(true);
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent=new Intent(VarietyGroupSummery.this, RaSurveyDashboard.class);
                startActivity(intent);
                finish();
            }
        });
        if(new InternetCheck(VarietyGroupSummery.this).isOnline())
        {
            new GetDataList().execute(factory,SupvcodeFrom,SupvcodeTo,VillageFrom,VillageTo,FromDate,ToDate);
        }
        else
        {
            Toast.makeText(VarietyGroupSummery.this,getString(R.string.oops_connect_your_internet),Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private class GetDataList extends AsyncTask<String, Void, Void> {
        String message;
        private ProgressDialog dialog = new ProgressDialog(VarietyGroupSummery.this);

        @Override
        protected void onPreExecute() {
            super.onPreExecute();
            dialog = ProgressDialog.show(VarietyGroupSummery.this, getString(R.string.app_name),
                    "Please Wait ", true);
            dialog.show();
        }
        ...
        protected Void doInBackground(String... params) {
            try {
                ...
                SoapObject request1 = new SoapObject(APIUrl.NAMESPACE, APIUrl.method_SupervisorVarietyGroupArea);
                ...
                androidHttpTransport.call(APIUrl.SOAP_ACTION_SupervisorVarietyGroupArea, envelope);
                if (envelope.bodyIn instanceof SoapFault) {
                    SoapFault sf = (SoapFault) envelope.bodyIn;
                    message = sf.getMessage();
                } else {
                    SoapObject result = (SoapObject) envelope.bodyIn;
                    message = result.getPropertyAsString("SupervisorVarietyGroupAreaResult").toString();
                }
        ...
        protected void onPostExecute(Void result) {
            ...
            try{
                ...
                if(jsonArray.length()==0)
                {
                        ...
                        if(i>0)
                        {
                            ...
                            if(jsonObject1.getInt("SUPVCODE") == jsonObject.getInt("SUPVCODE"))
                            {
                                if(jsonObject1.getInt("VCODE") == jsonObject.getInt("VCODE"))
                                {
                                    ...
                                    dataMap.put("ATNAME",jsonObject.getString("VRGROUP"));
                    ...
                    GridLayoutManager manager = new GridLayoutManager(VarietyGroupSummery.this, 1, RecyclerView.VERTICAL, false);
                    //recyclerView.setLayoutManager(manager);
                    //gridLayoutManager=new GridLayoutManager(MainActivity.this,8,RecyclerView.HORIZONTAL,true);
                    recyclerView.setHasFixedSize(true);
                    recyclerView.setLayoutManager(manager);
                    ListGetVarietyGroupSummeryAdapter listGetCropTypeSummeryAdapter =new ListGetVarietyGroupSummeryAdapter(VarietyGroupSummery.this,jsonArray1);
            ...
            catch (JSONException e)
            {
                if(dialog.isShowing())
                    dialog.dismiss();
                new AlertDialogManager().AlertPopUpFinish(VarietyGroupSummery.this,message);
