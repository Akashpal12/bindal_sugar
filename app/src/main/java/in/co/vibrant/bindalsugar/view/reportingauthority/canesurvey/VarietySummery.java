...
import in.co.vibrant.bindalsugar.adapter.ListGetVarietySummeryAdapter;
import in.co.vibrant.bindalsugar.model.CaneTypeSummeryModel;
...
public class VarietySummery extends AppCompatActivity{
    ...
    List <CaneTypeSummeryModel> dashboardData;
    String TAG="";
    Spinner spinner;
    ...
    Toolbar toolbar;
    String factory,SupvcodeFrom,SupvcodeTo,VillageFrom,VillageTo,vrtfrom,vrtto,FromDate,ToDate;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        vrtfrom=getIntent().getExtras().getString("vrtfrom");
        vrtto=getIntent().getExtras().getString("vrtto");
        FromDate=getIntent().getExtras().getString("DateFrom");
        ToDate=getIntent().getExtras().getString("DateTo");
        setTitle(getString(R.string.MENU_VARIETY_SUMMARY));
        toolbar.setTitle(getString(R.string.MENU_VARIETY_SUMMARY));
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setDisplayShowHomeEnabled(true);
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent=new Intent(VarietySummery.this, RaSurveyDashboard.class);
                startActivity(intent);
                finish();
            }
        });
        if(new InternetCheck(VarietySummery.this).isOnline())
        {
            new GetDataList().execute(factory,SupvcodeFrom,SupvcodeTo,VillageFrom,VillageTo,vrtfrom,vrtto,FromDate,ToDate);
        }
        else
        {
            Toast.makeText(VarietySummery.this,getString(R.string.oops_connect_your_internet),Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private class GetDataList extends AsyncTask<String, Void, Void> {
        String message;
        private ProgressDialog dialog = new ProgressDialog(VarietySummery.this);

        @Override
        protected void onPreExecute() {
            super.onPreExecute();
            dialog = ProgressDialog.show(VarietySummery.this, getString(R.string.app_name),
                    "Please Wait ", true);
            dialog.show();
        }
        ...
        protected Void doInBackground(String... params) {
            try {
                ...
                SoapObject request1 = new SoapObject(APIUrl.NAMESPACE, APIUrl.method_GetSupervisorVarietyArea);
                ...
                request1.addProperty("vrtfrom", params[5]);
                request1.addProperty("vrtto", params[6]);
                request1.addProperty("DateFrom", params[7]);
                request1.addProperty("DateTo", params[8]);
                ...
                androidHttpTransport.call(APIUrl.SOAP_ACTION_GetSupervisorVarietyArea, envelope);
                if (envelope.bodyIn instanceof SoapFault) {
                    SoapFault sf = (SoapFault) envelope.bodyIn;
                    message = sf.getMessage();
                } else {
                    SoapObject result = (SoapObject) envelope.bodyIn;
                    message = result.getPropertyAsString("GetSupervisorVarietyAreaResult").toString();
                }
        ...
        protected void onPostExecute(Void result) {
            ...
            try{
                ...
                if(jsonArray.length()==0)
                {
                        ...
                        if(i>0)
                        {
                            ...
                            if(jsonObject1.getInt("SUPVCODE") == jsonObject.getInt("SUPVCODE"))
                            {
                                if(jsonObject1.getInt("VCODE") == jsonObject.getInt("VCODE"))
                                {
                                    ...
                                    dataMap.put("ATNAME",jsonObject.getString("VRNAME"));
                    ...
                    List<Object> supData1=new ArrayList<>();
                    try{
                        JSONArray villArray=new JSONArray(gson.toJson(villData1));
                        ...
                        for(int i=0;i<villArray.length();i++) {
                            ...
                            if (i > 0) {
                                ...
                                if (jsonObject1.getInt("SUPVCODE") == jsonObject.getInt("SUPVCODE")) {
                                    Map<String,Object> supData=new HashMap<>();
                                    supData.put("villcode",jsonObject.getString("villcode"));
                                    supData.put("villname",jsonObject.getString("villname"));
                                    supData.put("villData",jsonObject.getJSONArray("villData"));
                    ...
                    GridLayoutManager manager = new GridLayoutManager(VarietySummery.this, 1, RecyclerView.VERTICAL, false);
                    //recyclerView.setLayoutManager(manager);
                    //gridLayoutManager=new GridLayoutManager(MainActivity.this,8,RecyclerView.HORIZONTAL,true);
                    recyclerView.setHasFixedSize(true);
                    recyclerView.setLayoutManager(manager);
                    ListGetVarietySummeryAdapter listGetCropTypeSummeryAdapter =new ListGetVarietySummeryAdapter(VarietySummery.this,jsonArray1);
            ...
            {
                //AlertPopUp(message);
                new AlertDialogManager().AlertPopUpFinish(VarietySummery.this,message);
                if(dialog.isShowing())
                    dialog.dismiss();
