...
import in.co.vibrant.bindalsugar.view.supervisor.StaffTargetGrowerReport;


public class StaffTargetReportListAdapter extends RecyclerView.Adapter<StaffTargetReportListAdapter.MyHolder> {
    ...
    public StaffTargetReportListAdapter(Context context, List <StaffTargetModel> arrayList) {
    ...
    public MyHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View layout= LayoutInflater.from(parent.getContext()).inflate(R.layout.single_item_report_target_list,null);
    ...
    public void onBindViewHolder(MyHolder holder, final int position) {
        try {
            ...
            if(position==0)
            {
                holder.txt1.setText("Village");
                holder.txt2.setText("Target Area");
                holder.txt3.setText("MANUAL AREA ONDATE/TODATE");
                holder.txt4.setText("GPS AREA ONDATE/TODATE");
            }
            else
            {
                holder.txt1.setText(arrayList.get(position).getVillageCode()+"/"+arrayList.get(position).getVillageName());
                holder.txt2.setText(arrayList.get(position).getTargetArea());
                holder.txt3.setText(arrayList.get(position).getOnManualArea()+"/"+arrayList.get(position).getToManualArea());
                holder.txt4.setText(arrayList.get(position).getOnGpsArea()+"/"+arrayList.get(position).getToGpsArea());
            }


            if(position>0)
            {
                holder.txt5.setText("DETAILS");
                holder.txt5.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        //
                        Intent intent=new Intent(context, StaffTargetGrowerReport.class);
                        intent.putExtra("target",arrayList.get(position).getTargetCode());
                        intent.putExtra("VillCode",arrayList.get(position).getVillageCode());
                        intent.putExtra("VillName",arrayList.get(position).getVillageName());
