...
public class StaffAddGrowerFormDetailActivity extends AppCompatActivity {
    ...
    private List<SaveGrowerDocumentModel> saveGrowerDocumentModels = new ArrayList<>();
   ...
   Context context;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context= StaffAddGrowerFormDetailActivity.this;
    ...
    private void init() {
        ...
        try {
            if (db != null) {
                saveGrowerDocumentModels.clear();
                saveGrowerDocumentModels.addAll(db.getAllGrowerInputData(UtilFile.villageCode, UtilFile.growerCode));                Log.d(TAG, "init: " + "><><><><" + saveGrowerDocumentModels.size());
                for (int i = 0; i < saveGrowerDocumentModels.size(); i++) {
                    String village = saveGrowerDocumentModels.get(i).getAgriVillageName().toLowerCase();
                    String mVill = village.substring(0, 1).toUpperCase() + village.substring(1);
                    tv_Name.setText(saveGrowerDocumentModels.get(i).getAgriGrowerName());

                    String gFather = saveGrowerDocumentModels.get(i).getAgriGrowerFather().toLowerCase();
                    String mGFather = gFather.substring(0, 1).toUpperCase() + gFather.substring(1);
                    tv_growerFatherName.setText("Father name : "+mGFather);
                    tv_growerCode.setText("Grower code : " + saveGrowerDocumentModels.get(i).getAgriGrowerCode());
                    tv_VillagCode.setText("Village code : "+saveGrowerDocumentModels.get(i).getAgriVillageCode());
                    ...
                    Log.d(TAG, "init: " + saveGrowerDocumentModels.size());
                }
        ...
        btn_next.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent=new Intent(StaffAddGrowerFormDetailActivity.this, StaffAddGrowerFormActivity.class);
                saveGrowerDocumentModels=db.getAllGrowerInputData();
                String agriInputId="";
                agriInputId = "" + saveGrowerDocumentModels.get(0).getAgri_input_Id();
