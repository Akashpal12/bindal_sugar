...
import in.co.vibrant.bindalsugar.adapter.ListGetRemainShareAdapter;
...
public class RemainShare extends AppCompatActivity{
    ...
    Toolbar toolbar;
    String factory,SupvcodeFrom,SupvcodeTo,VillageFrom,VillageTo,vrtfrom,vrtto;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle("Remain Summary");
        toolbar.setTitle("Remain Summary");
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setDisplayShowHomeEnabled(true);
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent=new Intent(RemainShare.this, RaSurveyDashboard.class);
                startActivity(intent);
                finish();
            }
        });
        if(new InternetCheck(RemainShare.this).isOnline())
        {
            new GetDataList().execute(factory,SupvcodeFrom,SupvcodeTo,VillageFrom,VillageTo,vrtfrom,vrtto);
        }
        else
        {
            Toast.makeText(RemainShare.this,getString(R.string.oops_connect_your_internet),Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private class GetDataList extends AsyncTask<String, Void, Void> {
        String message;
        private ProgressDialog dialog = new ProgressDialog(RemainShare.this);

        @Override
        protected void onPreExecute() {
            super.onPreExecute();
            dialog = ProgressDialog.show(RemainShare.this, getString(R.string.app_name),
                    "Please Wait ", true);
            dialog.show();
        }
        ...
        protected Void doInBackground(String... params) {
            try {
                ...
                SoapObject request1 = new SoapObject(APIUrl.NAMESPACE, APIUrl.method_GetRemainShare);
                ...
                androidHttpTransport.call(APIUrl.SOAP_ACTION_GetRemainShare, envelope);
                if (envelope.bodyIn instanceof SoapFault) {
                    SoapFault sf = (SoapFault) envelope.bodyIn;
                    message = sf.getMessage();
                } else {
                    SoapObject result = (SoapObject) envelope.bodyIn;
                    message = result.getPropertyAsString("GetRemainShareResult").toString();
                }
        ...
        protected void onPostExecute(Void result) {
            ...
            try{
                ...
                if(jsonArray.length()==0)
                {
                        ...
                        for(int i=0;i<villArray.length();i++) {
                            ...
                            if (i > 0) {
                                ...
                                if (jsonObject1.getInt("SUPVCODE") == jsonObject.getInt("SUPVCODE")) {
                                    ...
                                    supData.put("PLOTNO",jsonObject.getString("PLOTNO"));
                                    supData.put("REMAINPERCENT",jsonObject.getString("REMAINPERCENT"));
                    ...
                    GridLayoutManager manager = new GridLayoutManager(RemainShare.this, 1, RecyclerView.VERTICAL, false);
                    //recyclerView.setLayoutManager(manager);
                    //gridLayoutManager=new GridLayoutManager(MainActivity.this,8,RecyclerView.HORIZONTAL,true);
                    recyclerView.setHasFixedSize(false);
                    recyclerView.setLayoutManager(manager);
                    ListGetRemainShareAdapter listGetRemainShareAdapter =new ListGetRemainShareAdapter(RemainShare.this,jsonArray1);
                    recyclerView.setAdapter(listGetRemainShareAdapter);
                }
                if (dialog.isShowing())
                    dialog.dismiss();
            }
            catch (JSONException e)
            {
                if(dialog.isShowing())
                    dialog.dismiss();
                new AlertDialogManager().AlertPopUpFinish(RemainShare.this,message);
                //AlertPopUpFinish(message);
            }catch (Exception e)
            {
                new AlertDialogManager().AlertPopUpFinish(RemainShare.this,"Error:"+e.toString());
            }
        }
    }
