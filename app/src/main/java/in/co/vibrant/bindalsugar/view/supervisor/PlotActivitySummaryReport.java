...
import in.co.vibrant.bindalsugar.adapter.PlotActivitySummaryReportAdapter;
...
public class PlotActivitySummaryReport extends AppCompatActivity {

    DBHelper dbh;
    Context context;
    List<PlotActivitySummaryReportModel> modelList = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.plot_activity_summary_report);
        Toolbar toolbar = findViewById(R.id.toolbar);
        setSupportActionBar(toolbar);
        setTitle(getString(R.string.MENU_PLOT_ACTIVITY_SUM_REPORT));
        toolbar.setTitle(getString(R.string.MENU_PLOT_ACTIVITY_SUM_REPORT));
        ...
        context = PlotActivitySummaryReport.this;
        ...
        fromdate_select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                DatePickerDialog dpd = new DatePickerDialog(PlotActivitySummaryReport.this, new DatePickerDialog.OnDateSetListener() {
                    @Override
    ...
    class PlotSummaryReport extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            try {
                String url = APIUrl.BASE_URL_CANE_DEVELOPMENT + "/PlotActivitySummaryReport_New";
            ...
            try {
                ...
                if (jsonObject.getString("STATUS").equalsIgnoreCase("OK")) {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        PlotActivitySummaryReportModel model=new PlotActivitySummaryReportModel();
                        model.setSNO(object.getString("SNO"));//DATE
                        model.setVILLNAME(object.getString("VILLNAME"));//DATE
                        model.setPLVILL(object.getString("PLVILL"));//VILLCODE
                        model.setOD_PLOTS(object.getString("OD_PLOTS"));//VILLNAME
                        model.setOD_AREA(object.getString("OD_AREA"));//GROCODE
                        model.setTD_PLOTS(object.getString("TD_PLOTS"));//GRONAME
                        model.setTD_AREA(object.getString("TD_AREA"));//Bleaching Powder @  384.00
                        model.setFROM_DATE(fromdate_select.getText().toString());//Bleaching Powder @  384.00
                        model.setTO_DATE(todate_selects.getText().toString());//Bleaching Powder @  384.00
                        model.setS_CODE(userDetailsModels.get(0).getCode()+" / "+userDetailsModels.get(0).getName());//Bleaching Powder @  384.00


                        double OdDate = Double.parseDouble(object.getString("OD_PLOTS"));
                        plotsonDate += OdDate;

                        double odArea = Double.parseDouble(object.getString("OD_AREA"));
                        areaonDate += odArea;

                        double toDatePlots= Double.parseDouble(object.getString("TD_PLOTS"));
                        plotstodate += toDatePlots;

                        double toDateArea = Double.parseDouble(object.getString("TD_AREA"));
                        areatodate += toDateArea;
                    ...
                    PlotActivitySummaryReportAdapter adapter = new PlotActivitySummaryReportAdapter(context, modelList);
                    ...
                    plots_onDate.setText(new DecimalFormat("#0.000").format(plotsonDate));
                    area_onDate.setText(new DecimalFormat("#0.000").format(areaonDate));
                    plots_todate.setText(new DecimalFormat("#0.000").format(plotstodate));
                    area_todate.setText(new DecimalFormat("#0.000").format(areatodate));
