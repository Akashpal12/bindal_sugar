...
import in.co.vibrant.bindalsugar.adapter.RaNonVarietyWithZoneAdapter;
import in.co.vibrant.bindalsugar.adapter.RaVarietyWithZoneAdapter;
...
public class RaNonCircleWithZoneReport extends AppCompatActivity  {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context= RaNonCircleWithZoneReport.this;
        setTitle(getString(R.string.MENU_ZONE_WITH_CIRCLE_REPORT));
        toolbar.setTitle(getString(R.string.MENU_ZONE_WITH_CIRCLE_REPORT));
        ...
        fromdate_select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                DatePickerDialog dpd = new DatePickerDialog(RaNonCircleWithZoneReport.this, new DatePickerDialog.OnDateSetListener() {
                    @Override
    ...
    class TargetList extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            try {
                String url= APIUrl.BASE_URL_CANE_DEVELOPMENT +"/ZONECIRCLEWISEAREA1";
        ...
        protected void onPostExecute(String sResponse) {
            List<Object> targetFinal=new ArrayList<>();
            JSONArray jsonArray1=new JSONArray();
            try {
                if (dialog.isShowing())
                    dialog.dismiss();
                JSONArray jsonArray = new JSONArray(Content);
                String currentVarietyName="";
                Gson gson=new Gson();

                if(jsonArray.length()>0)
                {
                    JSONObject jsonObject1=jsonArray.getJSONObject(0);
                    currentVarietyName = jsonObject1.getString("CIRNAME");
                    List<Map<String,String>> target=new ArrayList<>();
                    for (int i = 0; i < jsonArray.length(); i++)
                    {
                        JSONObject jsonObject=jsonArray.getJSONObject(i);
                        if(currentVarietyName.equalsIgnoreCase(jsonObject.getString("CIRNAME")))
                        {
                            Map<String,String> map =new HashMap<String, String>();
                            map.put("CIRNAME",jsonObject.getString("CIRNAME"));
                            map.put("CIRCODE",jsonObject.getString("CIRCODE"));
                        ...
                        else
                        {
                            if(target.size()>0)
                            {
                                ...
                                for(int j=0;j<target.size();j++)
                                {
                                    ...
                                    map1.put("CIRNAME",target.get(j).get("CIRNAME"));
                                    map1.put("CIRCODE",target.get(j).get("CIRCODE"));
                ...
                RaNonVarietyWithZoneAdapter stockSummeryAdapter =new RaNonVarietyWithZoneAdapter(context,jsonArray1);
                ...
                recyclerView.setLayoutManager(manager);
                //RaSelfTargetAdapter stockSummeryAdapter =new RaSelfTargetAdapter(context,staffTargetModalList);
                RaVarietyWithZoneAdapter stockSummeryAdapter =new RaVarietyWithZoneAdapter(context,jsonArray1);
                recyclerView.setAdapter(stockSummeryAdapter);
            }catch (Exception e) {
                System.out.println("Error : " + e.toString());
                Log.e(e.getClass().getName(), e.getMessage(), e);
