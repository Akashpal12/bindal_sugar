...
*/
   // List<FactoryModel> factoryModelListSpinner;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context= TotalKmActivity.this;
        dbh = new DBHelper(this);
        db = new DBHelper(this).getWritableDatabase();
        //transportDetailsList=dbh.getTransportDetails();
        setTitle(getString(R.string.MENU_TRUCK_TRAVEL));
        toolbar.setTitle(getString(R.string.MENU_TRUCK_TRAVEL));
   ...
   /* private class GetTruckList extends AsyncTask<String, Void, Void> {
    ...
    private class GetTruckDetails extends AsyncTask<String, Void, Void> {
        ...
        protected Void doInBackground(String... params) {
            try {
                String imei=new GetDeviceImei(context).GetDeviceImeiNumber();
                SoapObject request1 = new SoapObject(APIUrl.NAMESPACE, APIUrl.method_GetTotalTravel);
                request1.addProperty("trans_code", params[0]);
                ...
                HttpTransportSE androidHttpTransport = new HttpTransportSE(APIUrl.BASE_URL, 200000);
                androidHttpTransport.debug = true;
                androidHttpTransport.call(APIUrl.SOAP_ACTION_GetTotalTravel, envelope);
                if (envelope.bodyIn instanceof SoapFault) {
                    SoapFault sf = (SoapFault) envelope.bodyIn;
                    message = sf.getMessage();
                } else {
                    SoapObject result = (SoapObject) envelope.bodyIn;
                    message = result.getPropertyAsString("GetTotalTravelResult").toString();
                }
            } catch (SecurityException e) {
                Log.e("Exception", e.getMessage());
                message = e.getMessage();
                ...
                if(jsonObject.getString("API_STATUS").equalsIgnoreCase("OK"))
                {
                    ...
                    if(jsonArray.length()>0)
                    {
                        for(int i=0;i<jsonArray.length();i++)
                        {
                            ...
                            truckDetails.setTotalKm(object.getString("KM"));
                    ...
                    recyclerView.setLayoutManager(manager);
                    TotalKmAdapter stockSummeryAdapter =new TotalKmAdapter(context,truckDetailsList);
