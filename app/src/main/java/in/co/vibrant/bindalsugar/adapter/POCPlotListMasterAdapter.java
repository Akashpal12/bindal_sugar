...
import in.co.vibrant.bindalsugar.view.supervisor.PUCPlotActivity;


public class POCPlotListMasterAdapter extends RecyclerView.Adapter<POCPlotListMasterAdapter.MyHolder> {

    private Context context;
    List <POCGrowerFinderModel> arrayList;
    AlertDialog AlertdialogBox;


    public POCPlotListMasterAdapter(Context context, List <POCGrowerFinderModel> arrayList) {
    ...
    public void onBindViewHolder(@NonNull MyHolder holder,  final int position) {
        ...
        holder.tvCount.setText(arrayList.get(position).getVILLAGE_ID());
        holder.tv_grower_name.setText(arrayList.get(position).getG_NAME()+" ("+arrayList.get(position).getSUVTYPE()+")");
        holder.tv_father_name.setText(arrayList.get(position).getGF_NAME());
        holder.tv_plot_village.setText(arrayList.get(position).getV_NAME()+" - "+arrayList.get(position).getVILLAGE_ID());
        holder.tv_grower_village.setText(arrayList.get(position).getG_VILLAGE()+" - "+arrayList.get(position).getG_CODE());
        holder.tv_variety.setText("Variety: "+arrayList.get(position).getVERIETYNAME());
        holder.tv_cane_type.setText("Cane Type: "+arrayList.get(position).getCROPTYPENAME());
        holder.tv_cane_area.setText("Plot Area: "+arrayList.get(position).getPLOTAREA());
        holder.tv_variety_group.setText("Grower Area: "+arrayList.get(position).getCANEAREA());
        holder.tv_get_grower_mobile.setText("Grower Mobile : "+arrayList.get(position).getG_MOBNO());
        holder.tv_PLOT_NO.setText("Plot No : "+arrayList.get(position).getPLOT_SERIAL_NO());
        holder.rl_tv_grower_number.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                    try {
                        if (arrayList.get(position).getG_MOBNO().toString().length()==10) {
                            Intent intent = new Intent(Intent.ACTION_CALL);
                            intent.setData(Uri.parse("tel:" +arrayList.get(position).getG_MOBNO()));
   ...
   /* private void setTable(MyHolder holder, JSONArray jsonArray)
    ...
    public void openMenu(int position) {
        try {
            ...
            View mView = LayoutInflater.from(context).inflate(R.layout.dialogue_puc_menu, null);
            dialogbilder.setView(mView);
            AlertdialogBox = dialogbilder.create();
            RelativeLayout plot_activity = mView.findViewById(R.id.puc_plot);
            plot_activity.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Intent intent = new Intent(context, PUCPlotActivity.class);
                    intent.putExtra("V_CODE", arrayList.get(position).getVILLAGE_ID());
                    intent.putExtra("V_NAME", arrayList.get(position).getV_NAME());
                    intent.putExtra("G_CODE", arrayList.get(position).getG_CODE());
                    intent.putExtra("G_FATHER", arrayList.get(position).getGF_NAME());
                    intent.putExtra("PLOT_SR_NO", arrayList.get(position).getPLOT_SERIAL_NO());
                    intent.putExtra("GSRNO", arrayList.get(position).getG_CODE());
                    intent.putExtra("CANE_TYPE",arrayList.get(position).getCROPTYPENAME());
                    intent.putExtra("PLOT_VILL", arrayList.get(position).getVILLAGE_ID());
                    intent.putExtra("PLOT_VILL_NAME", arrayList.get(position).getV_NAME());
                   // intent.putExtra("AREA", arrayList.get(position).getCaneArea());
                    intent.putExtra("SHARE_AREA", arrayList.get(position).getPLOTAREA());
                    intent.putExtra("G_NAME", arrayList.get(position).getG_NAME());
                    intent.putExtra("SURTYPE", arrayList.get(position).getSUVTYPE());
                    intent.putExtra("OLDSEAS", arrayList.get(position).getSEAS());
                    intent.putExtra("MOBILE", arrayList.get(position).getG_MOBNO());
                    intent.putExtra("LAT", arrayList.get(position).getLat());
                    intent.putExtra("LNG", arrayList.get(position).getLang());

                   // intent.putParcelableArrayListExtra("QuestionsExtra", (ArrayList<? extends Parcelable>) mQuestionsList);
